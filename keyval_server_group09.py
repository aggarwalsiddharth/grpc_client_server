...
class KeyValueServicer(keyval_pb2_grpc.KeyValueServicer):
	...
	def Delete(self, request, context):
		...
		# if not key:
		# 		error_status = keyval_pb2.Status(server_id=server_id,ok= False,error=f"Delete aborted. Invalid delete proto")
		...
		if key and current_version:
			...
			# print('Yo')
			error_status = keyval_pb2.Status(server_id=server_id,ok= False,error=f"Key not present {key}")
			return keyval_pb2.DeleteResponse(status = error_status,key = None, deleted_value = None , deleted_version = None)
			...
			if current_version == 0:
				...
				## Current version 0 
				error_status = keyval_pb2.Status(server_id=server_id,ok= False,error='Invalid delete proto key: '+ str(key))
				...
				error_status = keyval_pb2.Status(server_id=server_id,ok= False,error='Invalid delete proto current_version: '+ str(current_version))
				return keyval_pb2.DeleteResponse(status = error_status,key = key, deleted_value = None , deleted_version = None)
...
if __name__ == '__main__':
	...
	parser.add_argument('-id','--server_id',default=1)
	parser.add_argument('-delay','--write_delay',default=0)
	parser.add_argument('-port','--port',default='50050')
